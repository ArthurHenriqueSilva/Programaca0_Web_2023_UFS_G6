...
def consulta_distribuicao_imigrantes_pais(pais_filtro):
    with app.app_context():
        ...
        return sorted(distribuicao, key=lambda tup: tup[1])
...
def consulta_mes_mais_atrativo(uf_filtro, classificacao_filtro):
    with app.app_context():
        registros = db_session.query(Registro.mes, db.func.sum(Registro.qtd).label('Total')).filter(Registro.uf == uf_filtro, Registro.Classificacao == classificacao_filtro).all() \
            .group_by(Registro.mes) \
            .order_by(db.desc('Total')) \
            .first()
...
def distribuicao_imigrantes_pais():
    distribuicao = consulta_distribuicao_imigrantes_pais()
    return jsonify({'Total_Fronteirico': distribuicao[0][1], 'Total_Provisorio': distribuicao[1][1], 'Total_Residente': distribuicao[2][1], 'Total_Temporario': distribuicao[3][1]})
